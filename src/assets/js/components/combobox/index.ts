...
class HSComboBox extends HSBasePlugin<IComboBoxOptions> implements IComboBox {
    ...
    constructor(el: HTMLElement, options?: IComboBoxOptions, events?: {}) {
        ...
        this.selected = this.value =
            (this.el.querySelector('[data-hs-combo-box-input]') as HTMLInputElement).value ?? ''
        this.isOpened = false
    ...
    private isTextExists(el: HTMLElement, val: string[]): boolean {
        ...
        return Array.from(el.querySelectorAll('[data-hs-combo-box-search-text]')).some((elI: HTMLElement) =>
            lowerCased.includes(elI.getAttribute('data-hs-combo-box-search-text').toLowerCase())
        )
    }

    private isTextExistsAny(el: HTMLElement, val: string): boolean {
        return Array.from(el.querySelectorAll('[data-hs-combo-box-search-text]')).some((elI: HTMLElement) =>
            elI.getAttribute('data-hs-combo-box-search-text').toLowerCase().includes(val.toLowerCase())
        )
    }

    private valuesBySelector(el: HTMLElement) {
        return Array.from(el.querySelectorAll('[data-hs-combo-box-search-text]')).reduce(
            (acc: any, cur: HTMLElement) => [...acc, cur.getAttribute('data-hs-combo-box-search-text')],
            []
        )
    }
    ...
    private jsonItemsRender(items: any) {
        items.forEach((item: never, index: number) => {
            ...
            newItem.querySelectorAll('[data-hs-combo-box-output-item-field]').forEach((el) => {
                ...
                const hideIfEmpty = el.hasAttribute('data-hs-combo-box-output-item-hide-if-empty')
    ...
    private groupTabsRender() {
        ...
        tabDef.addEventListener('click', () => {
            ...
            const selectedTab = this.tabs.find(
                (elI: HTMLElement) => elI.getAttribute('data-hs-combo-box-group-title') === this.selectedGroup
            )
    ...
    private setItemsVisibility() {
        ...
        if (this.groupingType === 'default') {
            this.output.querySelectorAll('[data-hs-combo-box-group-title]').forEach((el: HTMLElement) => {
    ...
    static autoInit() {
        ...
        document.querySelectorAll('[data-hs-combo-box]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
    ...
    static onArrow(isArrowUp = true) {
        ...
        if (target) {
            ...
            if (!current) items[0].classList.add('hs-combo-box-output-item-highlighted')
    ...
    static onEnter(evt: Event) {
        ...
        const opened = window.$hsComboBoxCollection.find(
            (el) =>
                ...
                (evt.target as HTMLElement).closest('[data-hs-combo-box]') === el.element.el
        )

        const link: HTMLAnchorElement = opened.element.el.querySelector('.hs-combo-box-output-item-highlighted a')
