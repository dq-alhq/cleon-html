...
class HSStepper extends HSBasePlugin<{}> implements IStepper {
    ...
    // Nav
    private buildNav() {
        this.el.querySelectorAll('[data-hs-stepper-nav-item]').forEach((el) => this.addNavItem(el as HTMLElement))
    ...
    static autoInit() {
        ...
        document.querySelectorAll('[data-hs-stepper]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
