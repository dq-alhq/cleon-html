...
class HSOverlay extends HSBasePlugin<{}> implements IOverlay {
    ...
    constructor(el: HTMLElement, options?: IOverlayOptions, events?: {}) {
        ...
        this.animationTarget = this?.overlay?.querySelector('.hs-overlay-animation-target') || this.overlay
    ...
    private buildBackdrop() {
        ...
        ;(backdrop as HTMLElement).setAttribute('data-hs-overlay-backdrop-template', '')
    ...
    public open() {
        ...
        const toggles = document.querySelectorAll(`[data-hs-overlay="#${this.overlay.id}"]`)
    ...
    public close(forceClose = false) {
        if (this.isLayoutAffect) document.body.classList.remove('hs-overlay-body-open')
    ...
    static autoInit() {
        ...
        document.querySelectorAll('[data-hs-overlay]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
    ...
    static onTab(target: ICollectionItem<HSOverlay>, focusableElements: HTMLElement[]) {
...
const setBackdropZIndexResizeFn = () => {
    if (
        ...
        !window.$hsOverlayCollection.find((el) => el.element.overlay.classList.contains('opened'))
    )
        return false

    const overlays = window.$hsOverlayCollection.filter((el) => el.element.overlay.classList.contains('opened'))
