...
class PinInput extends BaseComponent<IPinInputOptions> implements IPinInput {
    private readonly items: NodeListOf<HTMLElement> | null
    ...
    private init() {
        this.createCollection(window.$PinInputCollection, this)
    ...
    static getInstance(target: HTMLElement | string, isInstance?: boolean) {
        const elInCollection = window.$PinInputCollection.find(
    ...
    static autoInit() {
        if (!window.$PinInputCollection) window.$PinInputCollection = []

        document.querySelectorAll('[data-pin-input]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$PinInputCollection.find((elC) => (elC?.element?.el as HTMLElement) === el)) new PinInput(el)
        })
    }
}
...
declare global {
    interface Window {
        PinInput: Function
        $PinInputCollection: ICollectionItem<PinInput>[]
    }
}
