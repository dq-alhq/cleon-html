...
class HSSearchByJson extends HSBasePlugin<ISearchByJsonOptions, HTMLInputElement> implements ISearchByJson {
    ...
    static autoInit() {
        if (!window.$hsSearchByJsonCollection) window.$hsSearchByJsonCollection = []

        document
            .querySelectorAll('[data-hs-search-by-json]:not(.--prevent-on-load-init)')
            .forEach((el: HTMLInputElement) => {
                if (!window.$hsSearchByJsonCollection.find((elC) => (elC?.element?.el as HTMLElement) === el))
