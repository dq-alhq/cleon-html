...
class HSToggleCount extends HSBasePlugin<IToggleCountOptions> implements IToggleCount {
    ...
    static autoInit() {
        ...
        document.querySelectorAll('[data-hs-toggle-count]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$hsToggleCountCollection.find((elC) => (elC?.element?.el as HTMLElement) === el))
