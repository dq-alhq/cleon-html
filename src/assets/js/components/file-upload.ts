...
class FileUpload extends BaseComponent<IFileUploadOptions> implements IFileUpload {
    private readonly concatOptions: IFileUploadOptions
    private readonly previewTemplate: string
        ...
        this.previewTemplate =
			...
			<div class="mb-2 flex justify-between items-center">
				<div class="flex items-center gap-x-3">
					<span class="size-8 flex justify-center items-center border border-gray-200 text-gray-500 rounded-lg dark:border-neutral-700 dark:text-neutral-500" data-file-upload-file-icon></span>
					<div>
						<p class="text-sm font-medium text-gray-800 dark:text-white">
							<span class="truncate inline-block max-w-[300px] align-bottom" data-file-upload-file-name></span>.<span data-file-upload-file-ext></span>
						</p>
						<p class="text-xs text-gray-500 dark:text-neutral-500" data-file-upload-file-size></p>
					</div>
				</div>
				<div class="inline-flex items-center gap-x-2">
					<button type="button" class="text-gray-500 hover:text-gray-800 dark:text-neutral-500 dark:hover:text-neutral-200" data-file-upload-remove>
						<svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"></path><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path><line x1="10" x2="10" y1="11" y2="17"></line><line x1="14" x2="14" y1="11" y2="17"></line></svg>
					</button>
				</div>
			</div>
			<div class="flex items-center gap-x-3 whitespace-nowrap">
				<div class="flex w-full h-2 bg-gray-200 rounded-full overflow-hidden dark:bg-neutral-700" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" data-file-upload-progress-bar>
					<div class="flex flex-col justify-center rounded-full overflow-hidden bg-blue-600 text-xs text-white text-center whitespace-nowrap transition-all duration-500 file-upload-complete:bg-green-600 dark:bg-blue-500" style="width: 0" data-file-upload-progress-bar-pane></div>
				</div>
    ...
    private init() {
        this.createCollection(window.$FileUploadCollection, this)
    ...
    private initDropzone() {
        ...
        if (pseudoTriggers.length)
            pseudoTriggers.forEach((el) => {
                el.onclick = () => {
                    if (this.concatOptions?.clickable) (this.concatOptions?.clickable as HTMLButtonElement).click()
                }
            })
    }
    ...
    private onAddFile(file: DropzoneFile) {
        ...
        if (this.singleton && this.dropzone.files.length > 1) this.dropzone.removeFile(this.dropzone.files[0])
    ...
    private previewAccepted(file: DropzoneFile) {
        ...
        if (this.dropzone.files.length > 0 && this.concatOptions.autoHideTrigger) trigger.style.display = 'none'
    ...
    private onRemoveFile() {
        ...
        if (this.dropzone.files.length === 0 && this.concatOptions.autoHideTrigger) trigger.style.display = ''
    }
    ...
    private onUploadProgress(file: DropzoneFile, progress: number) {
        ...
        const progressBarPane = previewElement.querySelector('[data-file-upload-progress-bar-pane]') as HTMLElement
        const progressBarValue = previewElement.querySelector('[data-file-upload-progress-bar-value]') as HTMLElement
    ...
    private createIcon(ext: string) {
        ...
        classToClassList(this.extensions[ext]?.class ? this.extensions[ext].class : this.extensions.default.class, icon)
    ...
    static getInstance(target: HTMLElement | string, isInstance?: boolean) {
        const elInCollection = window.$FileUploadCollection.find(
    ...
    static autoInit() {
        if (!window.$FileUploadCollection) window.$FileUploadCollection = []

        document.querySelectorAll('[data-file-upload]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$FileUploadCollection.find((elC) => (elC?.element?.el as HTMLElement) === el))
                new FileUpload(el)
        })
    }
}
...
declare global {
    interface Window {
        FileUpload: Function
        $FileUploadCollection: ICollectionItem<FileUpload>[]
    }
}

window.addEventListener('load', () => {
    if (document.querySelectorAll('[data-file-upload]:not(.--prevent-on-load-init)').length) {
        if (typeof _ === 'undefined') console.error('FileUpload: Lodash is not available, please add it to the page.')
        if (typeof Dropzone === 'undefined')
            console.error('FileUpload: Dropzone is not available, please add it to the page.')
    }
