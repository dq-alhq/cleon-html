...
class Carousel extends BaseComponent<ICarouselOptions> implements ICarousel {
    ...
    private init() {
        this.createCollection(window.$CarouselCollection, this)
    ...
    private calculateWidth() {
        ...
        this.slides.forEach((el: { style: { width: string } }) => {
    ...
    private addCurrentClass() {
        this.slides.forEach(
            (el: { classList: { add: (arg0: string) => void; remove: (arg0: string) => void } }, i: number) => {
    ...
    static getInstance(target: HTMLElement | string, isInstance?: boolean) {
        const elInCollection = window.$CarouselCollection.find(
    ...
    static autoInit() {
        if (!window.$CarouselCollection) window.$CarouselCollection = []

        document.querySelectorAll('[data-carousel]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$CarouselCollection.find((elC) => (elC?.element?.el as HTMLElement) === el)) new Carousel(el)
        })
    }
}
...
declare global {
    interface Window {
        Carousel: Function
        $CarouselCollection: ICollectionItem<Carousel>[]
    }
}
...
window.addEventListener('resize', () => {
    if (!window.$CarouselCollection) return false

    window.$CarouselCollection.forEach((el) => {
