...
import { SELECT_ACCESSIBILITY_KEY_SET } from '../utils/constants'
import BaseComponent from './base'

interface ISingleOptionOptions {
    description: string
    icon: string
}
...
class Select extends BaseComponent<ISelectOptions> implements ISelect {
    ...
    private readonly extraMarkup: string | string[] | null
    ...
    constructor(el: HTMLElement, options?: ISelectOptions) {
        ...
        this.viewport =
            typeof concatOptions?.viewport !== 'undefined' ? document.querySelector(concatOptions?.viewport) : null
        ...
        this.toggleSeparators = { ...defaultToggleSeparators, ...concatOptions?.toggleSeparators }
        ...
        this.searchNoResultClasses =
            concatOptions?.searchNoResultClasses || 'px-4 text-sm text-gray-800 dark:text-neutral-200'
    ...
    private init() {
        this.createCollection(window.$SelectCollection, this)
    ...
    private buildWrapper() {
        ...
        this.wrapper.classList.add('select', 'relative')
    ...
    private buildExtraMarkup() {
        ...
        if (Array.isArray(this.extraMarkup)) this.extraMarkup.forEach((el) => appendMarkup(el))
    ...
    private buildToggle() {
        ...
        if (this.isMultiple) {
            ...
            this.toggleTextWrapper.innerHTML = this.getItemByValue(this.value as string)?.title || this.placeholder
        }
    ...
    private setToggleIcon() {
        ...
        if (icon) {
            const img = htmlToElement(this.getItemByValue(this.value as string)?.options?.icon || '')
            icon.append(img)
    ...
    private buildTagsItem(val: string) {
        ...
        let template: any, title: any, remove: any, icon: null | HTMLElement
    ...
    private buildTagsInput() {
        ...
        this.tagsInput.addEventListener(
            'input',
            debounce((evt: InputEvent) => this.searchOptions((evt.target as HTMLInputElement).value))
        )
    ...
    private buildSearch() {
        let input: { append: (arg0: HTMLInputElement) => void }
    ...
    private onSelectOption(val: string) {
        ...
        dispatch('change.select', this.el, this.value)
    ...
    private setNewValue() {
        if (this.mode === 'tags') {
            ...
            if (this.value.length) this.toggleTextWrapper.innerHTML = this.stringFromValue()
    ...
    private selectSingleItem() {
        const selectedOption = Array.from(this.el.children).find((el) => this.value === (el as HTMLOptionElement).value)
    ...
    public open() {
        const currentlyOpened = window?.$SelectCollection?.find((el) => el.element.isOpened) || null
    ...
    public close() {
        ...
        this.dropdown.querySelector('.select-option-highlighted')?.classList.remove('select-option-highlighted')
        this.isOpened = false
    }
    ...
    static getInstance(target: HTMLElement | string, isInstance?: boolean) {
        const elInCollection = window.$SelectCollection.find(
    ...
    static autoInit() {
        if (!window.$SelectCollection) window.$SelectCollection = []

        document.querySelectorAll('[data-select]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$SelectCollection.find((elC) => (elC?.element?.el as HTMLElement) === el)) {
                const data = el.getAttribute('data-select')
        ...
        if (window.$SelectCollection) {
            window.addEventListener('click', (evt) => {
                const evtTarget = evt.target

                Select.closeCurrentlyOpened(evtTarget as HTMLElement)
            })

            document.addEventListener('keydown', (evt) => Select.accessibility(evt))
        }
    }
    ...
    static closeCurrentlyOpened(evtTarget: HTMLElement | null = null) {
        if (!evtTarget.closest('.select.active')) {
            const currentlyOpened = window.$SelectCollection.filter((el) => el.element.isOpened) || null
    ...
    static accessibility(evt: KeyboardEvent) {
        const target = window.$SelectCollection.find((el) => el.element.isOpened)
    ...
    static onArrow(isArrowUp = true) {
        ...
        if (target) {
            ...
            const current = dropdown.querySelector('.select-option-highlighted') || dropdown.querySelector('.selected')
    ...
    static onEnter(evt: Event) {
        ...
        if (window.$SelectCollection.find((el) => el.element.el === select)) {
            const opened = window.$SelectCollection.find((el) => el.element.isOpened)
            const target = window.$SelectCollection.find((el) => el.element.el === select)
...
declare global {
    interface Window {
        Select: Function
        $SelectCollection: ICollectionItem<Select>[]
    }
}
...
document.addEventListener('scroll', () => {
    if (!window.$SelectCollection) return false
