...
import { TABS_ACCESSIBILITY_KEY_SET } from '../utils/constants'

class Tabs extends BaseComponent<{}> implements ITabs {
    ...
    constructor(el: HTMLElement, options?: {}, events?: {}) {
        ...
        this.currentContentId = this.current.getAttribute('data-tab')
    ...
    private init() {
        this.createCollection(window.$TabsCollection, this)
    ...
    static getInstance(target: HTMLElement | string, isInstance?: boolean) {
        const elInCollection = window.$TabsCollection.find(
    ...
    static autoInit() {
        if (!window.$TabsCollection) window.$TabsCollection = []

        document
            .querySelectorAll('[role="tablist"]:not(select):not(.--prevent-on-load-init)')
            .forEach((el: HTMLElement) => {
                if (!window.$TabsCollection.find((elC) => (elC?.element?.el as HTMLElement) === el)) new Tabs(el)
            })

        if (window.$TabsCollection) document.addEventListener('keydown', (evt) => Tabs.accessibility(evt))
    }

    static open(target: HTMLElement) {
        const elInCollection = window.$TabsCollection.find((el) =>
    ...
    static onArrow(isOpposite = true) {
        ...
        const targetInCollection = window.$TabsCollection.find((el) => el.element.el === target)
...
declare global {
    interface Window {
        Tabs: Function
        $TabsCollection: ICollectionItem<Tabs>[]
    }
}
