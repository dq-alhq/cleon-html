...
class Stepper extends BaseComponent<{}> implements IStepper {
    ...
    private init() {
        this.createCollection(window.$StepperCollection, this)
    ...
    static getInstance(target: HTMLElement | string, isInstance?: boolean) {
        const elInCollection = window.$StepperCollection.find(
    ...
    static autoInit() {
        if (!window.$StepperCollection) window.$StepperCollection = []

        document.querySelectorAll('[data-stepper]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$StepperCollection.find((elC) => (elC?.element?.el as HTMLElement) === el)) new Stepper(el)
        })
    }
}
...
declare global {
    interface Window {
        Stepper: Function
        $StepperCollection: ICollectionItem<Stepper>[]
    }
}
