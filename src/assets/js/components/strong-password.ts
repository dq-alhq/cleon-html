...
import { classToClassList, dispatch, htmlToElement, isEnoughSpace } from '../utils'
...
class StrongPassword extends BaseComponent<IStrongPasswordOptions> implements IStrongPassword {
    ...
    private init() {
        this.createCollection(window.$StrongPasswordCollection, this)
    ...
    private build() {
        ...
        ;(this.target as HTMLInputElement).addEventListener('input', (evt: InputEvent) => {
    ...
    private buildHints() {
        this.weakness = (this.hints as HTMLElement).querySelector('[data-strong-password-hints-weakness-text]') || null
        this.rules =
            Array.from((this.hints as HTMLElement).querySelectorAll('[data-strong-password-hints-rule-text]')) || null

        this.rules.forEach((rule) => {
            const ruleValue = rule.getAttribute('data-strong-password-hints-rule-text')
    ...
    private buildWeakness() {
        ...
        ;(this.target as HTMLInputElement).addEventListener('input', () => setTimeout(() => this.setWeaknessText()))
    }

    private buildRules() {
        this.setRulesText()
        ;(this.target as HTMLInputElement).addEventListener('input', () => setTimeout(() => this.setRulesText()))
    }

    private setWeaknessText() {
        const weaknessText = this.weakness.getAttribute('data-strong-password-hints-weakness-text')
    ...
    static getInstance(target: HTMLElement | string) {
        const elInCollection = window.$StrongPasswordCollection.find(
    ...
    static autoInit() {
        if (!window.$StrongPasswordCollection) window.$StrongPasswordCollection = []

        document.querySelectorAll('[data-strong-password]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$StrongPasswordCollection.find((elC) => (elC?.element?.el as HTMLElement) === el)) {
                const data = el.getAttribute('data-strong-password')
                const options: IStrongPasswordOptions = data ? JSON.parse(data) : {}

                new StrongPassword(el, options)
            }
        })
    }
}
...
declare global {
    interface Window {
        ...
        $StrongPasswordCollection: ICollectionItem<StrongPassword>[]
    }
}

window.addEventListener('load', () => {
    StrongPassword.autoInit()
})

document.addEventListener('scroll', () => {
    if (!window.$StrongPasswordCollection) return false

    const target = window.$StrongPasswordCollection.find((el) => el.element.isOpened)
