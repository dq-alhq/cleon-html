...
class HSDropdown extends HSBasePlugin<{}, IHTMLElementPopper> implements IDropdown {
    ...
    static autoInit() {
        ...
        document.querySelectorAll('.hs-dropdown:not(.--prevent-on-load-init)').forEach((el: IHTMLElementPopper) => {
    ...
    static closeCurrentlyOpened(evtTarget: HTMLElement | null = null, isAnimated = true) {
        ...
        let currentlyOpened = parent
            ? window.$hsDropdownCollection.filter(
                  (el) =>
                      el.element.el.classList.contains('open') &&
                      el.element.menu.closest('.hs-dropdown').parentElement.closest('.hs-dropdown') === parent
              )
        ...
        if (
            evtTarget &&
            evtTarget.closest('.hs-dropdown') &&
            getClassPropertyAlt(evtTarget.closest('.hs-dropdown'), '--auto-close') === 'inside'
        ) {
