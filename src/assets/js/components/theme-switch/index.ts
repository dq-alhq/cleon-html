...
class HSThemeSwitch extends HSBasePlugin<IThemeSwitchOptions> implements IThemeSwitch {
    ...
    public setAppearance(theme = this.theme, isSaveToLocalStorage = true, isSetDispatchEvent = true) {
        ...
        if (isSetDispatchEvent) window.dispatchEvent(new CustomEvent('on-hs-appearance-change', { detail: theme }))
    }
    ...
    static autoInit() {
        ...
        const toggleObserveSystemTheme = (el: HSThemeSwitch) => {
            if (localStorage.getItem('hs_theme') === 'auto') el.addSystemThemeObserver()
            else el.removeSystemThemeObserver()
        }

        document.querySelectorAll('[data-hs-theme-switch]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$hsThemeSwitchCollection.find((elC) => (elC?.element?.el as HTMLElement) === el)) {
