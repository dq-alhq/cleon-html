...
class InputNumber extends BaseComponent<IInputNumberOptions> implements IInputNumber {
    ...
    private init() {
        this.createCollection(window.$InputNumberCollection, this)
    ...
    static getInstance(target: HTMLElement | string, isInstance?: boolean) {
        const elInCollection = window.$InputNumberCollection.find(
    ...
    static autoInit() {
        if (!window.$InputNumberCollection) window.$InputNumberCollection = []

        document.querySelectorAll('[data-input-number]:not(.--prevent-on-load-init)').forEach((el: HTMLElement) => {
            if (!window.$InputNumberCollection.find((elC) => (elC?.element?.el as HTMLElement) === el))
                new InputNumber(el)
        })
    }
}
